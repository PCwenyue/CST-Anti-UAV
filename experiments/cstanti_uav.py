...
class ExperimentCSTAntiUAV(object):
    ...
    def __init__(self, root_dir, subset,
        ...
        super(ExperimentCSTAntiUAV, self).__init__()
        self.subset = subset
        ...
        self.att_fig_name = ['OC','OV', 'SV', 'TC','DBC', 'CDB','FM', 
                        'TS', 'SS', 'MS', 'NS']
    ...
    def report(self, trackers, plot_curves=True, plot_attcurves=True):
        ...
        for trackerid, tracker in enumerate(trackers):
            ...
            for ii in range(len(self.att_name)):
                ...
                if trackerid == 0:
                    self.att_name[ii] = self.att_name[ii]
                ...
                # 计算该属性中序列的平均 SA
                avg_att_sa = np.mean([overall_SA[s] for s in att_ids]) if len(att_ids) > 0 else 0
        ...
        for trackerid, tracker in enumerate(trackers):
            name = tracker['name']
            for ii, att_name in enumerate(self.att_name):
                avg_att_sa = performance[name][self.att_fig_name[ii]]['avg_att_sa']
                text = '[Attribute] %20s %20s: %.04f' % (name, att_name, avg_att_sa)
                with open(sa_file, 'a', encoding='utf-8') as f:
                    f.write(text)
                    f.write('\n')
                print(text)
    ...
    def plot_curves(self, tracker_names):
        ...
        for i, name in enumerate(tracker_names):
            ...
            lines.append(line)
            if i == 0:
                legends.append(r'$\mathbf{%s: [%.4f]}$' % (name, performance[name][key]['success_score']))
            else:
                legends.append('%s: [%.4f]' % (name, performance[name][key]['success_score']))
        # for Default trackers
        legend = ax.legend(lines, legends, loc='lower left', ncol=2, prop={'size': 11},frameon=True)
        ...
        ax.set(xlabel='Overlap threshold',
               ylabel='Success rate',
               xlim=(0, 1), ylim=(0, 1.0),
               title='Success plots of OPE——CST Anti-UAV')
        ax.set_title('Success plots of OPE — CST Anti-UAV', fontweight='bold')
        ...
        for i, name in enumerate(tracker_names):
            ...
            lines.append(line)
            if i == 0:
                legends.append(r'$\mathbf{%s: [%.4f]}$' % (name, performance[name][key]['precision_score']))
            else:
                legends.append('%s: [%.4f]' % (name, performance[name][key]['precision_score']))
        # for Default trackers
        legend = ax.legend(lines, legends, loc='lower right', ncol=2,prop={'size':11}, frameon=True)
        ...
        ax.set(xlabel='Location error threshold',
               ...
               title='Precision plots of OPE — CST Anti-UAV')
        ax.set_title('Precision plots of OPE — CST Anti-UAV', fontweight='bold')
        # ax.xaxis.label.set_size(17)
        # ax.yaxis.label.set_size(17)
        ax.grid(True)
        fig.tight_layout()
    ...
    def plot_attcurves(self, tracker_names, att_name, att_key):
        ...
        succ_file = os.path.join(report_dir, 'success_plots_of_'+att_key+'.png')
        prec_file = os.path.join(report_dir, 'precision_plots_of_'+att_key+'.png')
        ...
        for i, name in enumerate(tracker_names):
            ...
            lines.append(line)
            if i == 0:
                legends.append(r'$\mathbf{%s: [%.4f]}$' % (name, performance[name][att_key]['att_success_score']))
            else:
                legends.append('%s: [%.4f]' % (name, performance[name][att_key]['att_success_score']))
        # for Default trackers
        legend = ax.legend(lines, legends, loc='lower left', ncol=2,prop={'size': 11}, frameon=True)
        ...
        for i, name in enumerate(tracker_names):
            ...
            lines.append(line)
            if i == 0:
                legends.append(r'$\mathbf{%s: [%.4f]}$' % (name, performance[name][att_key]['att_precision_score']))
            else:
                legends.append('%s: [%.4f]' % (name, performance[name][att_key]['att_precision_score']))
        # for Default trackers
        legend = ax.legend(lines, legends, loc='lower right', ncol=2,prop={'size':11}, frameon=True)
        # matplotlib.rcParams.update({'font.size': 7.4})
